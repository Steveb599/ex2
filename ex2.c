	...
	*/
	...
	case 4:
	{
		...
		if (notPrime)
		{
			...
			printf("This one brings joy: %d\n");
		break;
	case 5:
		int sum5, num5, numsDigit5 = 0, temp5;
		printf("Enter a number:\n");
		scanf("%d", &num5);
		while (num5 <= 0)
		{
	...
	case 6:
	{
		...
		while (smile <= 0 || cheer <= 0 || cheer == smile)
		{
			scanf("%*[^\n]");
			scanf("%*c");
			printf("Only 2 different positive numbers in the given foramt are allowed for the festival, please try again:\n");
			scanf("smile: %d, cheer: %d", &smile, &cheer);
		}
